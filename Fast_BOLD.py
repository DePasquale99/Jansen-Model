...
def balloon(x):
    ...
    Function for integration of the BOLD signal model; 
    Takes:
    x. state of the system at the previous step of interation, composed of
        u: synaptic activity of the brain region
        s: vasodilatatory signal
        f_in: incoming blood flux
        v: volume of the blood vessel
        q: desoxyhemoglobine concentration
    Returns: dx, vector containing the finite differences between the previous and next step of integration
        for the 4 variables that are to be integrated
...
def Euler_balloon(t, x, *args):
    ...
    for i in range(N):
        ...
        ds[i] = epsilon*U[i][round(t)] -s[i]/tau_s -(f_in[i] -1)/tau_f
...
def euler_balloon(t, x, *args):
    ...
    X = np.reshape(x, (N,4))
    X = np.append(X, np.transpose([U[round(t)]]), axis= 1)
...
def RK4_integration(input):
    ...
    output = RK4(euler_balloon, X0.flatten(), timepoints, input).reshape((len(timepoints), N, 4))
    X = np.array([output[:,2,:],output[:,3,:]])
    print(np.shape(X))
    signal = np.apply_along_axis(BOLD, 0, X)
    return signal
